...
public class Uploader
{
  ...
  */
    public Hashtable upFile(HttpContext cxt, string pathbase, string[] filetype, int size)
    {
        ...
        uploadpath = HttpContext.Current.Request.PhysicalApplicationPath + pathbase; //cxt.Server.MapPath(pathbase);//获取文件上传路径
        ...
        try
        {
            ...
            if (state == "SUCCESS")
            {
                ...
                URL = ConfigurationManager.AppSettings["domain"].ToString()+"/"+pathbase + filename;
            }
        }
        catch (Exception e)
        {
